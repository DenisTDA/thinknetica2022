class RailWay
  ...
  def do_list
    loop do
      ...
      send(COMMAND_DO[user_choice(menu['main'].keys)])
    end
  end
  ...
  COMMAND_4DO = {
    ...
    '2' => :list_trains,
    '3' => :list_routes,
    '4' => :list_trains_on_station,
    '5' => :list_stations_with_trains
  }
  ...
  def menu_create
    loop do
      ...
      choice = user_choice(menu['create'].keys)
      choice == '0' ? break : send(COMMAND_1DO[choice])
    end
  end
  ...
  def menu_change
    loop do
      ...
      choice = user_choice(menu['change'].keys)
      choice == '0' ? break : send(COMMAND_2DO[choice])
    end
  end
  ...
  def menu_list
    loop do
      ...
      choice = user_choice(menu['list'].keys)
      choice == '0' ? break : send(COMMAND_4DO[choice])
  ...
  def list_stations(stations = self.stations)
    if stations.any?
      puts 'Список станций'
  ...
  def list_trains(trains = self.trains)
    if self.trains.any?
      puts 'Список поездов'
  ...
  def list_routes(routes = self.routes)
    if self.routes.any?
