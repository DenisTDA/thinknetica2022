class RailWay
  ...
  def initialize
    ...
    @menu = {"main" => {"1" => "Input data" , "2" => "Change data", 
      ...
      '2' => {'1'=>'Add station to route', '2'=>'Delete station from route', 
            '3'=>'Set route to  train', '4'=>'Add and delete carriages station'},
      '3' => {'1'=>''},
      '4' => {'1' => 'Stations list', '2' => 'Trains list', 
        '3' => 'Routes list', '4' => 'Trains on station'},
      }
  end
  ...
  def create_station
    station_names = []
    self.stations.each {|station| station_names << station.name} 
    list_stations(self.stations)
    print "Input station's name: "
    station_name = item_check(station_names)
  ...
  def item_check(items)
    begin
      ...
      print "Allready exist, try once more: " if items.include?(item)
  ...
  def create_train
    ...
    self.trains.each {|train| train_numbers << train.number} 
    list_trains(self.trains) if self.trains.size > 0 
    print "Input train's number: "
    train_number = item_check(train_numbers)
    choice_type = choose_type_train 
    type[choice_type].nil? ? return : self.trains << type[choice_type].new(train_number)
    puts print_train(self.trains.last)
    puts "Train #{self.trains.last.number} was created"
    pause
  end

  def create_route
    if self.stations.size > 1
      list_routes(self.routes)
      list_stations(self.stations)
  ...
  def add_station
    unless (index_route = select_route) == END_OPERATION
      stations = stations_for_add(self.routes[index_route])
      station = select_station(self.routes[index_route], stations)
      unless station == END_OPERATION
        insert_action(index_route, station)
      else
        puts "Station did't add to route"
      end
    end
  end

  def select_route
    if list_routes(self.routes)
      print "Choose the route for add (1 - #{self.routes.size}, Exit - 0): "
      index_route = choose_item(self.routes.size)
      puts print_route(self.routes[index_route])
      index_route
    else
      puts "Problems with route!"
      pause
      END_OPERATION
    end
  end

  def stations_for_add(route)
    stations = []
  ...
  def insert_action(index_route, station)
    self.routes[index_route].insert(station)
    puts "Station <#{station.name}> was added to route #{print_route(routes[index_route])}"
  end
  ...
  def del_station
    unless (index_route = select_route) == END_OPERATION
      ...
      unless station == END_OPERATION
        ...
        puts "Station was't remove frome route"
      end
    end
  end

  def set_route
    index_route = select_route
    index_train = select_train
    if index_route == END_OPERATION || index_train == END_OPERATION
  ...
  def select_train
    unless list_trains(self.trains) == END_OPERATION
      print "Выбирайте поезд, которому будет назначен выбранный маршрут (1 - #{self.trains.size}): "
  ...
  def list_stations(stations)
    if stations.any? 
      puts "Список станций"
      stations.each.with_index(1) {|item, index| puts "#{index}. - #{stations[index-1].name}"} 
    else
      puts "Список пуст"
      END_OPERATION
    end
